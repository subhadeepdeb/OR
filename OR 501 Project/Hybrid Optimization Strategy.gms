...
idle_cost_factor 'Idle time cost factor' /0.5/;
...
Positive Variables
idle_hours(e,j) 'Idle hours for each employee type in each month';
...
idle_time_calculation(e,j) Idle Time Calculation;
...
objective.. Z =E= sum((i,j,k), X(i,j,k) * fuel_cost(i))
                ...
                + sum((e,j), idle_hours(e,j) * salary(e) / max_hours * idle_cost_factor);
...
working_hours_limit(e,j).. sum((i,k), X(i,j,k) * flight_duration(i) * crew_req(k,e)) + idle_hours(e,j) =E= staff(e,j) * max_hours;
...
idle_time_calculation(e,j).. idle_hours(e,j) =E= staff(e,j) * max_hours - sum((i,k), X(i,j,k) * flight_duration(i) * crew_req(k,e));

Model hybrid_optimization /all/;

Solve hybrid_optimization using mip minimizing Z;

Display X.l, leased_fleet.l, staff.l, hire.l, fire.l, idle_hours.l, Z.l;